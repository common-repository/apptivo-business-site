...
.awp_contactform_maindiv_'.esc_attr($hrjobsform['name']).'{max-width:700px;}
...
echo '<input type="hidden" value="'.esc_attr($hrjobsform['name']).'" name="awp_jobsformname" id="awp_jobsformname">';
echo '<div class="awformmain awsinglecolmn awp_hrjobsform_maindiv_'.esc_attr($hrjobsform['name']).'">';
...
foreach($formfields as $field)
{
    ...
    $captcha_class = (!empty($captcha_class)) ? $captcha_class : '';
	...
	switch($fieldtype)
	{
		...
		case "select":
			if($fieldid == 'country')
			{
				...
				echo '<input type="hidden" id="country_name" name="country_name" value="'.esc_attr($defaultcountryName).'">';
			}
...
echo '<div class="formsection"><div class="formrgt"><input type="'.esc_attr($hrjobsform['submit_button_type']).'" class="absp_jobapplicant_button_submit awp_hrjobsform_submit_'.esc_attr($hrjobsform['name']).'" '.html_entity_decode(esc_attr($button_value)).' name="awp_jobsform_submit" id="awp_jobsform_submit" /></div></div>';
echo '</div>';
