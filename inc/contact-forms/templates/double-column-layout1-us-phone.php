...
if( ! function_exists('validator_js_call_contactform_layout1_usphone_d') ) :
	function validator_js_call_contactform_layout1_usphone_d(){
...
add_action('wp_footer',"validator_js_call_contactform_layout1_usphone_d",100);
...
echo '<form id="'.esc_attr($contactform['name']).'_contactforms" name="'.esc_attr($contactform['name']).'_contactforms" class="abswpcfm" action="'.esc_attr($_SERVER['REQUEST_URI']).'" method="post">';
echo '<input type="hidden" value="'.esc_attr($contactform['name']).'" name="awp_contactformname" id="awp_contactformname">';
echo '<div class="awformmain awdblclm awp_contactform_maindiv_'.esc_attr($contactform['name']).'">';
foreach($formfields as $field)
{
	...
	$fieldtype=isset($field['type'])? strtolower($field['type']): '';
	...
	if($showtext != "")
	{
		...
		echo esc_attr($showtext).'</span></label>';
	}
	...
	switch($fieldtype){
		case "text":
			...
			{
				echo '<input type="text" name="'.esc_attr($fieldid).'" id="'.esc_attr($fieldid).'" value="'.esc_attr($postValue).'"  class="absp_contact_input_text'.esc_attr($validateclass).' field" field_value="'.esc_attr($field_value).'">';
			}
			else
			{
				echo '<div class="threefield"><input maxlength="3" size="3" type="text" name="'.esc_attr($fieldid).'1" id="'.esc_attr($fieldid).'1" value="'.esc_attr($postValue).'"  class="absp_contact_input_text'.esc_attr($validateclass).' field" field_value="'.esc_attr($field_value).'"></div>';
				echo '<div class="threefield pd0_10"><input maxlength="3" size="3" type="text" name="'.esc_attr($fieldid).'2" id="'.esc_attr($fieldid).'2" value="'.esc_attr($postValue).'"  class="absp_contact_input_text'.esc_attr($validateclass).'"></div>';
				echo '<div class="threefield"><input maxlength="4" size="4" type="text" name="'.esc_attr($fieldid).'3" id="'.esc_attr($fieldid).'3" value="'.esc_attr($postValue).'"  class="absp_contact_input_text'.esc_attr($validateclass).'"></div>';
			}
			break;
			
		case "textarea":
			echo '<textarea  name="'.esc_attr($fieldid).'" id="'.esc_attr($fieldid).'" value=""  class="absp_contact_textarea'.esc_attr($validateclass).' field1" size="50" field_value="'.esc_attr($field_value).'"></textarea>';
			break;
		...
		case "radio":
			...
			echo '<BR><label for="'.esc_attr($fieldid).'" generated="true" id="errorfield" class="error errorfield" style="display: none;">This field is required.</label>';
			break;
		...
		case "multiselect":
			if ($fieldid=='labels'){
				...
				echo '<BR><label for="'.esc_attr($fieldid).'[]" generated="true" id="errorfield" class="error" style="display: none;">This field is required.</label></div>';
			}
			break;
		case "captcha":
...
echo '<div class="form_section fullsection"><div class="formsect"><input type="checkbox" name="subscribe" id="subscribe" /><label>'.esc_attr($subscribe_to_newsletter).'</label></div></div>';
endif;
...
echo '<div class="form_section fullsection fullsecsub"><div class="formrgt"><input type="'.esc_attr($contactform['submit_button_type']).'" class="absp_contact_button_submit awp_contactform_submit_'.esc_attr($contactform['name']).'" '.html_entity_decode(esc_attr($button_value)).' name="awp_contactform_submit_'.esc_attr($contactform['name']).'"  id="awp_contactform_submit_'.esc_attr($contactform['name']).'" /></div></div>';
echo '</div>';
echo '</form><p>&nbsp;</p><br/>';
