...
.awformmain .formsection label.nne {display:none}
...
if( ! function_exists('validator_js_call_contactform_layout1_placeholder_s') ) :
function validator_js_call_contactform_layout1_placeholder_s(){
...
add_action('wp_footer',"validator_js_call_contactform_layout1_placeholder_s",100);
...
foreach($formfields as $field)
{
	...
	if($showtext != ""){
		if($fieldtype == "select" || $fieldtype == "radio" || $fieldtype == "checkbox" || $fieldtype == "multiselect"){ $shwcapt = ''; } else { $shwcapt = 'nne'; }
		echo '<label class="'.esc_attr($shwcapt).'">';
		if($required)
	...
	switch($fieldtype){
		case "text":
			echo '<input type="text" name="' . esc_attr($fieldid) . esc_attr($phone_validation) . '" id="' . esc_attr($fieldid) . '_id" value="' . esc_attr($postValue) . '"  placeholder="'.esc_attr($showtext).'" class="absp_contact_input_text' . esc_attr($validateclass) . ' field" field_value="'.esc_attr($field_value).'">';
			break;
		case "textarea":
			echo '<textarea  name="' . esc_attr($fieldid) . '" id="' . esc_attr($fieldid) . '" value="' . esc_attr($postValue) . '"  placeholder="'.esc_attr($showtext).'" class="absp_contact_textarea' . esc_attr($validateclass) . ' field1" size="50" field_value="'.esc_attr($field_value).'"></textarea>';
			break;
		case "select":
			if($fieldid == 'country'){
