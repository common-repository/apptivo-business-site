...
if( ! function_exists('validator_js_call_contactform_layout1_d') ) :
function validator_js_call_contactform_layout1_d(){
...
add_action('wp_footer',"validator_js_call_contactform_layout1_d",100);
...
echo '<form class="abswpcfm" id="'.esc_attr($contactform['name']).'_contactforms" name="'.esc_attr($contactform['name']).'_contactforms" action="'.esc_attr($_SERVER['REQUEST_URI']).'" method="post">';
...
foreach($formfields as $field)
{
	...
	switch($fieldtype){
		case "text":
			echo '<input type="text" name="'.esc_attr($fieldid.$phone_validation).'" id="'.esc_attr($fieldid).'_id" class="absp_contact_input_text'.esc_attr($validateclass).' field" field_value="'.esc_attr($field_value).'">';
			break;
		...
		case "multiselect":
			if ($fieldid=='labels'){
				...
				echo '<label for="'.esc_attr($fieldid).'[]" generated="true" id="errorfield" class="error errorfield" style="display: none;">This field is required.</label></div>';
			}
			break;
