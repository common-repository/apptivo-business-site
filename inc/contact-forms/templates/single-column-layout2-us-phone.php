<?php
/*
 Template Name:Single Column Layout2 with US Phone#
...
if( ! function_exists('validator_js_call_contactform_layout2_usphone_s') ) :
	function validator_js_call_contactform_layout2_usphone_s(){
...
add_action('wp_footer',"validator_js_call_contactform_layout2_usphone_s",100);
...
.awformmain .formsection label{width:35%;float:left;padding-right:10px;padding:5px 0;}
...
$jscript = (!empty($jscript)) ? $jscript : '';
echo html_entity_decode(esc_attr($jscript));
...
echo '<form id="'.esc_attr($contactform['name']).'_contactforms" class="abswpcfm awp_contact_'.esc_attr($contactform['name']).'" name="'.esc_attr($contactform['name']).'_contactforms" action="'.esc_attr($_SERVER['REQUEST_URI']).'" method="post">';
echo '<input type="hidden" value="'.esc_attr($contactform['name']).'" name="awp_contactformname" id="awp_contactformname">';
echo '<div class="awformmain awp_contactform_maindiv_'.esc_attr($contactform['name']).'">';
...
foreach($formfields as $field)
{
	...
	switch($fieldtype){
		case "text":
			...
			{
				echo '<input type="text" name="'.esc_attr($fieldid).'" id="'.esc_attr($fieldid).'" value=""  class="absp_contact_input_text'.esc_attr($validateclass).' field" field_value="'.esc_attr($field_value).'">';
			}
			else
			{
				echo '<div class="threefield"><input maxlength="3" size="3" type="text" name="'.esc_attr($fieldid).'1" id="'.esc_attr($fieldid).'1" value=""  class="absp_contact_input_text'.esc_attr($validateclass).'"></div>';
				echo '<div class="threefield pd0_10"><input maxlength="3" size="3" type="text" name="'.esc_attr($fieldid).'2" id="'.esc_attr($fieldid).'2" value=""  class="absp_contact_input_text'.esc_attr($validateclass).'"></div>';
				echo '<div class="threefield"><input maxlength="4" size="4" type="text" name="'.esc_attr($fieldid).'3" id="'.esc_attr($fieldid).'3" value=""  class="absp_contact_input_text'.esc_attr($validateclass).'"></div>';
			}
			break;
			
		case "textarea":
			echo '<textarea  name="'.esc_attr($fieldid).'" id="'.esc_attr($fieldid).'_id" value=""  class="absp_contact_textarea'.esc_attr($validateclass).' field1" size="50" field_value="'.esc_attr($field_value).'"></textarea>';
			break;
				
		case "select":
			if($fieldid == 'country'){
				echo  '<select  name="'.esc_attr($fieldid).'" id="'.esc_attr($fieldid).'"  class="absp_contact_select'.esc_attr($validateclass).' field" field_value="'.esc_attr($field_value).'">';
...
echo '<div class="formsection"><label><span>&nbsp;</span></label><div class="formrgt"><input type="'.esc_attr($contactform['submit_button_type']).'" class="absp_contact_button_submit awp_contactform_submit_'.esc_attr($contactform['name']).'" '.html_entity_decode(esc_attr($button_value)).' name="awp_contactform_submit_'.esc_attr($contactform['name']).'"  id="awp_contactform_submit_'.esc_attr($contactform['name']).'" /></div></div>';
echo '</div>';
echo '</form><p>&nbsp;</p>';
