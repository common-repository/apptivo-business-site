...
.awsinglecolmn .formsection label,.doublecolmn .formsection label{width: 100%;float: left;padding-right: 10px;}
...
if( ! function_exists('validator_js_call_contactform_layout1_usphone_s') ) :
function validator_js_call_contactform_layout1_usphone_s(){
...
add_action('wp_footer',"validator_js_call_contactform_layout1_usphone_s",100);
...
foreach($formfields as $field)
{
	...
	switch($fieldtype){
		case "text":
			...
			{
				echo '<div class="threefield"><input maxlength="3" size="3" type="text" name="'.esc_attr($fieldid).'1" id="'.esc_attr($fieldid).'1" value=""  class="absp_contact_input_text'.esc_attr($validateclass).' field" field_value="'.esc_attr($field_value).'"></div>';
...
echo '<div class="form_section"><div class="formrgt"><input type="'.esc_attr($contactform['submit_button_type']).'" class="absp_contact_button_submit awp_contactform_submit_'.esc_attr($contactform['name']).'" '.html_entity_decode(esc_attr($button_value)).' name="awp_contactform_submit_'.esc_attr($contactform['name']).'"  id="awp_contactform_submit_'.esc_attr($contactform['name']).'" /></div></div>';
echo '</div>';
echo '</form><p>&nbsp;</p><br />';
