...
/*
  Template Name: Toggle template- Testimonials Form
...
$_REQUEST['status'] = (!empty($_REQUEST['status'])) ? sanitize_text_field($_REQUEST['status']) : '';
...
#login-box .form_rgt_part{width:60% !important;float:left;}
#login-box .form_rgt_part input{width:100%;}
#login-box .form_rgt_part textarea{width:100%;}
...
.awp_fullview{clear:both;padding:5px;}
...
jQuery(document).ready(function() {
	jQuery('a.awp-testimonial-window').click(function() {
                jQuery('.awp_testimonial_form').toggle();
        });
});
</script>";
?>
...
$status_request = (!empty($_REQUEST['status'])) ? sanitize_text_field($_REQUEST['status']) : '';
if ($status_request) {
    $status_msg = $status_request;
    echo '<script type="text/javascript">
       jQuery(document).ready(function() {
    ...
    if ($status_msg == "Success") {
        ...
        $postValues= sanitize_text_field($_SESSION['POST_VALUES']);
    	echo '<script type="text/javascript">
       jQuery(document).ready(function() {
       jQuery("a.awp-testimonial-window").click();
       jQuery("fieldset.awp_testimonial_field label").first().before("<b> Please enter correct Verification code. </b>");
       });
        </script>';
...
if ($testimonialform['tmpl_button_type'] == "submit" && ($testimonialform['tmpl_button_val']) != "") {
    ...
    echo '<div class="awp-testimonial-post"><a href="javascript:void(0);" class="awp-testimonial-window"><button class="awp_tmpltype_btn">'.html_entity_decode(esc_attr($button_value)).'</button></a></div>';
} else {
    $button_value = $testimonialform['tmpl_button_val'];
    echo '<div class="awp-testimonial-post"><a href="javascript:void(0);" class="awp-testimonial-window"><img class="tmpltype_btn_img" src="'.html_entity_decode(esc_attr($button_value)).'"/></a></div>';
}
...
/* redirection URL */
$actual_link = SITE_URL.sanitize_text_field($_SERVER['REQUEST_URI']);
