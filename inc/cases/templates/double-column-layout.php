...
if( ! function_exists('validator_js_call_case_layout') ) :
	function validator_js_call_case_layout(){
...
add_action('wp_footer',"validator_js_call_case_layout",100);
...
.awformmain .fullsecsub .formrgt,.awformmain .fullsecsub .formrgt input{float:right;}
...
echo '<div class="awformmain awdblclm absp_business_main cases_outfrm_'.esc_attr($case_form['name']).'">';
...
foreach($cases_fields as $field){
    if (isset($field['showtext'])){
		...
		if($field['fieldid']=="status"){
			echo '<input type="hidden" name="awp_cases_status_name" value="'.esc_attr($field['options']).'"/>';
			echo '<input type="hidden" name="awp_cases_status_value" value="'.esc_attr($field['value']).'"/>';
		}
		$count++;
    }
}
...
echo '<div class="formsection fullsection fullsecsub"><label><span>&nbsp;</span></label><div class="formrgt">'.html_entity_decode(esc_attr($submit)).'</div></div>';
echo  '</div></form></div>';
